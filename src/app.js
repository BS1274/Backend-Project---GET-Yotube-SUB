...
//1---THIS ROUTE SHOWS ALL THE SUBSCRIBERS LIST WITH DETAILS---
...
//2----THIS ROUTE PROVIDES AN ARRAY OF ALL SUBSCRIBERS WITH ONLY TWO FIELDS, THEIR NAME AND SUBSCRIBED CHANNEL
...
app.get("/subscribers/names", async (req, res, next) => {
  try {
    ...
    const subscribers = await  schema.find({}, { name: 1, subscribedChannel: 1, _id: 0 });

    // Send the subscribers as a JSON response with a status of 200 (OK)
    res.status(200).json(subscribers);
  } catch (err) {
    // Set the response status to 400 (Bad Request) and pass the error to the error handling middleware
    res.status(400);
    next(err);
  }
});

//3--THIS ROUTE PROVIDES THE DETAILS OF SUBSCRIBER WITH THE GIVEN ID.---
